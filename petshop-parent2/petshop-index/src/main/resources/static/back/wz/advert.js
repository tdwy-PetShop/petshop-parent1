...
var vm=new Vue({
	...
	function member_stop1(id,dis){
	        ...
	        if(dis==1){
					...
					layer.confirm('确定解冻此广告？',function(index){
	          ...
	          //发异步把用户状态进行更改
					...
					layer.confirm('确定冻结此广告？',function(index){
					 var i=deleteUser(id,dis)
					 if(i>0){
...
function deleteUser(id,dis){
	...
	var list="";
	$.ajax({
		url:"http://localhost:8080/advert/updateDis",
		type:"post",
		data:{"id":id,"dis":dis},
	    dataType:"json",
		async:false,
...
function getData(){
		var list="";
		$.ajax({
			url:"http://localhost:8080/advert/selectAdmin",
			type:"post",
